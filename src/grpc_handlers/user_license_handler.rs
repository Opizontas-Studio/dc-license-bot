use prost::Message;
use sea_orm::{DatabaseConnection, EntityTrait, ActiveModelTrait, Set, ColumnTrait, QueryFilter, IntoActiveModel};
...
pub async fn handle_create_user_license(payload: &[u8], db: &DatabaseConnection) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    let request = CreateUserLicenseRequest::decode(payload)?;
    info!("Creating license for user {}: {}", request.user_id, request.license_name);
...
pub async fn handle_get_user_licenses(payload: &[u8], db: &DatabaseConnection) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    info!("Found {} licenses for user {}", licenses.len(), request.user_id);
...
pub async fn handle_update_user_license(payload: &[u8], db: &DatabaseConnection) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    let request = UpdateUserLicenseRequest::decode(payload)?;
    info!("Updating license {}", request.id);

    let mut license = user_licenses::Entity::find_by_id(request.id).one(db).await?
        .ok_or_else(|| format!("License with ID {} not found", request.id))?
        .into_active_model();

    if let Some(name) = request.license_name { license.license_name = Set(name); }
    if let Some(allow) = request.allow_redistribution { license.allow_redistribution = Set(allow); }
    if let Some(allow) = request.allow_modification { license.allow_modification = Set(allow); }
    if let Some(note) = request.restrictions_note { license.restrictions_note = Set(Some(note)); }
    if let Some(allow) = request.allow_backup { license.allow_backup = Set(allow); }
...
pub async fn handle_delete_user_license(payload: &[u8], db: &DatabaseConnection) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    let request = DeleteUserLicenseRequest::decode(payload)?;
    info!("Deleting license {}", request.id);

    let res = user_licenses::Entity::delete_by_id(request.id).exec(db).await?;

    let (success, message) = if res.rows_affected == 1 {
        (true, "License deleted successfully".to_string())
    } else {
        (false, format!("License with ID {} not found or could not be deleted", request.id))
    };
...
pub async fn handle_increment_usage_count(payload: &[u8], db: &DatabaseConnection) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    let request = IncrementUsageRequest::decode(payload)?;
    info!("Incrementing usage count for license {}", request.id);

    let license = user_licenses::Entity::find_by_id(request.id).one(db).await?
        .ok_or_else(|| format!("License with ID {} not found", request.id))?;
    ...
    let response = IncrementUsageResponse { new_usage_count: new_count };
    let mut buf = Vec::new();
    response.encode(&mut buf)?;
    Ok(buf)
}