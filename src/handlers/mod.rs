...
use serenity::all::{Context, FullEvent, Channel, ChannelType};
...
pub async fn poise_event_handler(
    ...
    match event {
        FullEvent::ThreadCreate { thread } => {
            // 检查是否是论坛类型频道中的线程
            if let Ok(parent_channel) = thread.parent_id.unwrap_or_default().to_channel(&ctx.http).await {
                if let Channel::Guild(guild_channel) = parent_channel {
