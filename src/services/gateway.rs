...
use registry::{
    ConnectionMessage, ConnectionRegister, Heartbeat, connection_message,
...
pub async fn start_gateway_client(
    ...
    let (conn_id_tx, conn_id_rx) = watch::channel::<Option<String>>(None);

    // 第一个消息发送注册
    tx.send(ConnectionMessage {
        message_type: Some(connection_message::MessageType::Register(
            ConnectionRegister {
    ...
    tokio::spawn(async move {
        let mut conn_id_rx = conn_id_rx;
        let mut interval = time::interval(Duration::from_secs(30));
        interval.tick().await; // consume the immediate tick to keep spacing consistent
        loop {
            let current_id = conn_id_rx.borrow().clone();
            let connection_id = match current_id {
                Some(connection_id) => connection_id,
                None => {
                    if conn_id_rx.changed().await.is_err() {
                        debug!("Heartbeat task exiting: connection ID channel closed");
                        break;
                    }
                    continue;
                }
            ...
            debug!(connection_id = %connection_id, "Sent heartbeat to gateway");
            ...
            match message_type {
                ...
                connection_message::MessageType::Status(status) => {
                    match registry::connection_status::StatusType::try_from(status.status) {
                        ...
                        Ok(registry::connection_status::StatusType::Error) => {
                            warn!(connection_id = %status.connection_id, message = %status.message, "Gateway reported connection error");
                        }
                        Err(_) => {
                            warn!(status = %status.status, "Received unknown connection status type");
                        }
                    }
                }
                _ => {
